...
class SELayer(nn.Module):
    """Squeeze-and-Excitation layer, a channel attention mechanism
    that adaptively recalibrates channel-wise feature responses.
    """
    def __init__(self, c, r=16):
        """
        Initialize SE layer with channel dimension c and reduction ratio r.
        
        Args:
            c (int): Number of input/output channels
            r (int): Reduction ratio for the hidden layer
        """
        super().__init__()
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output tensor with recalibrated channels
        """
        b, c, _, _ = x.shape
...
class BottleneckSE(nn.Module):
    """SE-enhanced Bottleneck layer with optional shortcut and group convolution for efficient feature extraction."""
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, r=16):
        """
        Initialize Bottleneck with Squeeze-and-Excitation.
        """
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        self.se = SELayer(c2, r=r)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        """Forward pass with SE enhancement."""
        return x + self.se(self.cv2(self.cv1(x))) if self.add else self.se(self.cv2(self.cv1(x)))


class C3SE(nn.Module):
    """CSP Bottleneck with 3 convolutions and SE attention"""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, r=16):
        """
        Initialize C3 with SE blocks.
        """
        super().__init__()
        
        # Handle arguments from YAML
        if isinstance(c2, list):
            # This is critical - unpack the arguments properly
            if len(c2) >= 5:  # If we have all [c2, shortcut, g, e, r]
                c2, shortcut, g, e, r = c2
            elif len(c2) == 4:  # If we have [c2, shortcut, g, e]
                c2, shortcut, g, e = c2
            elif len(c2) == 3:  # If we have [c2, shortcut, g]
                c2, shortcut, g = c2
            elif len(c2) == 2:  # If we have [c2, shortcut]
                c2, shortcut = c2
            else:  # If we only have [c2]
                c2 = c2[0]
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(BottleneckSE(c_, c_, shortcut, g, e=1.0, r=r) for _ in range(n)))
...
class ChannelAttention(nn.Module):
    def __init__(self, channels, reduction_ratio=16):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        # Remove max_pool and use only avg_pool (with doubled weight)
        self.fc = nn.Sequential(
    ...
    def forward(self, x):
        b, c, _, _ = x.shape
        # Use only average pooling (double the weight to compensate)
        avg_out = self.fc(self.avg_pool(x).view(b, c)) * 2
        return torch.sigmoid(avg_out).view(b, c, 1, 1)
...
class BottleneckCBAM(nn.Module):
    """CBAM-enhanced Bottleneck layer with optional shortcut and group convolution"""
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, r=16):
        """
        Initialize Bottleneck with CBAM
        
        Args:
            c1 (int): Number of input channels
            c2 (int): Number of output channels
            shortcut (bool): Whether to use shortcut connection
            g (int): Groups for conv2
            e (float): Channel expansion ratio
            r (int): CBAM reduction ratio
        """
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
    ...
    def forward(self, x):
        """Forward pass with CBAM enhancement"""
        return x + self.cbam(self.cv2(self.cv1(x))) if self.add else self.cbam(self.cv2(self.cv1(x)))


class C3CBAM(nn.Module):
    """CSP Bottleneck with 3 convolutions and CBAM attention"""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, r=16):
        """
        Initialize C3 with CBAM blocks
        
        Args:
            c1 (int): Number of input channels
            c2 (int): Number of output channels or list with [c2, shortcut, g, e, r]
            n (int): Number of Bottleneck layers
            shortcut (bool): Whether to use shortcut in Bottleneck
            g (int): Groups for Bottleneck
            e (float): Channel expansion ratio
            r (int): CBAM reduction ratio
        """
        super().__init__()
        
        # Check if c2 is a list and extract parameters
        if isinstance(c2, list) and len(c2) > 1:
            c2_val, shortcut, g, e, r = c2  # extract parameters if passed as a list
            c2 = c2_val  # reassign c2 to its actual value
        
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(BottleneckCBAM(c_, c_, shortcut, g, e=1.0, r=r) for _ in range(n)))
        
    def forward(self, x):
        """Forward pass"""
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))

# Existing Squeeze-and-Excitation and CBAM layers are already present
# Adding parallel SE and CBAM enhanced bottleneck and C3 layers

class BottleneckParallelAttention(nn.Module):
    """Bottleneck layer with parallel SE and CBAM attention mechanisms"""
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, r=16):
        """
        Initialize Bottleneck with parallel SE and CBAM
        ...
        Args:
            ...
            g (int): Groups for conv2
            e (float): Channel expansion ratio
            r (int): Reduction ratio for attention mechanisms
        """
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        ...
        self.se = SELayer(c2, r)
        self.cbam = CBAM(c2, r)
        
        self.add = shortcut and c1 == c2

    def forward(self, x):
        """Forward pass with parallel SE and CBAM attention"""
        # Compute base features
        base_features = self.cv2(self.cv1(x))
        
        # Apply parallel SE and CBAM
        se_features = self.se(base_features)
        cbam_features = self.cbam(base_features)
        
        # Combine features by averaging
        combined_features = (se_features + cbam_features) / 2
        
        # Apply shortcut if needed
        return x + combined_features if self.add else combined_features


class C3ParallelAttention(nn.Module):
    """CSP Bottleneck with parallel SE and CBAM attention"""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, r=16):
        """
        Initialize C3 with parallel SE and CBAM blocks
        
        Args:
            c1 (int): Number of input channels
            c2 (int): Number of output channels or list with configuration
            n (int): Number of Bottleneck layers
            shortcut (bool): Whether to use shortcut in Bottleneck
            g (int): Groups for Bottleneck
            e (float): Channel expansion ratio
        ...
        # Use the new parallel attention bottleneck
        self.m = nn.Sequential(
            *(BottleneckParallelAttention(c_, c_, shortcut, g, e=1.0, r=r) for _ in range(n))
        )
...
class SequentialSECBAM(nn.Module):
    def __init__(self, channels, reduction_ratio=16, spatial_kernel_size=7):
        super().__init__()
        # First apply SE
        self.se = SELayer(channels, reduction_ratio)
        
        # Then apply modified CBAM (using only avg pooling to avoid deterministic issues)
        self.channel_att = ChannelAttention(channels, reduction_ratio)
        self.spatial_att = SpatialAttention(spatial_kernel_size)
    ...
    def forward(self, x):
        ...
        # Then apply CBAM components
        x = x * self.channel_att(x)
        x = x * self.spatial_att(x)
        
        return x

class BottleneckSequential(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, r=16):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        self.attention = SequentialSECBAM(c2, r)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.attention(self.cv2(self.cv1(x))) if self.add else self.attention(self.cv2(self.cv1(x)))

class C3Sequential(nn.Module):
    """CSP Bottleneck with 3 convolutions and Sequential SE+CBAM attention"""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, r=16):
        """
        Initialize C3 with Sequential SE+CBAM blocks
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(BottleneckSequential(c_, c_, shortcut, g, e=1.0, r=r) for _ in range(n)))
...
class ESA(nn.Module):
    """Enhanced Spatial Attention layer as described in the paper.
    This layer is designed to refine spatial feature representations by focusing on salient areas.
    """
    def __init__(self, channel, reduction=16):
        super(ESA, self).__init__()
        # FC1: 1x1 convolution to capture low-level features
        self.fc1 = Conv(channel, channel, 1, 1)
        
        # FC2: 3x3 convolution followed by max pooling
        self.fc2_conv = Conv(channel, channel, 3, 1)
        self.fc2_pool = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
        
        # Fcgroup: Group of 3x3 convolutions with ReLU activations
        self.fcgroup = nn.Sequential(
            Conv(channel, channel, 3, 1),
            Conv(channel, channel, 3, 1),
            Conv(channel, channel, 3, 1)
        )
        
        # FC3: 1x1 convolution with sigmoid activation
        self.fc3 = nn.Sequential(
            Conv(channel, channel, 1, 1, act=False),
            nn.Sigmoid()
        )
        
    def forward(self, x):
        # Input from SPPF layer
        fsppf = x
        
        # FC1: Apply 1x1 convolution
        fc1 = self.fc1(fsppf)
        
        # FC2: Apply 3x3 convolution followed by max pooling
        fc2 = self.fc2_pool(self.fc2_conv(fc1))
        
        # Fcgroup: Apply sequence of 3x3 convolutions
        fcgroup = self.fcgroup(fc2)
        
        # FC3: Apply 1x1 convolution with sigmoid activation
        fc3 = self.fc3(fc1 + fcgroup)
        
        # Final output: Multiply with original input to apply attention
        return fc3 * fsppf + fsppf  # Residual connection for better gradient flow

class ESA_SE(nn.Module):
    """Enhanced Spatial Attention combined with Squeeze-and-Excitation"""
    def __init__(self, channel, reduction=16):
        super(ESA_SE, self).__init__()
        # ESA Components
        self.fc1 = Conv(channel, channel, 1, 1)
        self.fc2_conv = Conv(channel, channel, 3, 1)
        self.fc2_pool = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        # SE forward
        b, c, _, _ = esa_out.shape
        se_y = self.se_avg_pool(esa_out).view(b, c)
        se_y = self.se_fc(se_y).view(b, c, 1, 1)
        
        # Combine ESA and SE
        return se_y * esa_out + fsppf  # Apply SE to ESA output with residual connection

class SPP_ESA(nn.Module):
    """
    SPP (Spatial Pyramid Pooling) combined with ESA (Enhanced Spatial Attention)
    This module replaces SPPF with SPP and adds ESA afterward
    """
    def __init__(self, c1, c2, k=(5, 9, 13)):
        super().__init__()
        # Ensure k is a tuple/list
        k = (5, 9, 13) if isinstance(k, int) else k
        ...
        # ESA components
        self.esa_fc1 = Conv(c2, c2, 1, 1)
        self.esa_fc2_conv = Conv(c2, c2, 3, 1)
        self.esa_fc2_pool = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
        self.esa_fcgroup = nn.Sequential(
            Conv(c2, c2, 3, 1),
            Conv(c2, c2, 3, 1),
            Conv(c2, c2, 3, 1)
        )
        self.esa_fc3 = nn.Sequential(
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress torch 1.9.0 max_pool2d() warning
            spp_out = self.cv2(torch.cat([x] + [m(x) for m in self.m], 1))
        
        # ESA forward
        esa_input = spp_out
        fc1 = self.esa_fc1(esa_input)
        fc2 = self.esa_fc2_pool(self.esa_fc2_conv(fc1))
        fcgroup = self.esa_fcgroup(fc2)
        fc3 = self.esa_fc3(fc1 + fcgroup)
        
        # Apply attention and add residual connection
        return fc3 * esa_input + esa_input

class SPP_ESA_SE(nn.Module):
    """
    SPP (Spatial Pyramid Pooling) combined with ESA (Enhanced Spatial Attention) and SE (Squeeze-and-Excitation)
    This module combines three powerful techniques for feature enhancement
    """
    def __init__(self, c1, c2, k=(5, 9, 13), reduction=16):
        super().__init__()
        # Ensure k is a tuple/list
        k = (5, 9, 13) if isinstance(k, int) else k
    ...
    def forward(self, x):
        ...
        # Add residual connection to the combined output
        return se_out + esa_input